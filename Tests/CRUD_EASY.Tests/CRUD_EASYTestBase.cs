...
namespace CRUD_EASY.Tests
{
    public abstract class CRUD_EASYTestBase : AbpIntegratedTestBase<CRUD_EASYTestModule>
    {
        ...
        protected void UsingDbContext(Action<CRUD_EASYDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CRUD_EASYDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CRUD_EASYDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CRUD_EASYDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CRUD_EASYDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRUD_EASYDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CRUD_EASYDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRUD_EASYDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CRUD_EASYDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CRUD_EASYDbContext, Task<T>> func)
        {
            T result;
